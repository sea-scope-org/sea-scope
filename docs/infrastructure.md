...
| `AISSTREAM_BBOX`               | No       | Optional `southLat,westLon,northLat,eastLon` for AISStream (default Strait of Gibraltar `35.7,-6.0,36.3,-5.0`). Always subscribed; watch chart viewports ≤5° are added at runtime via `aisViewportReport` — see [architecture/maritime-watch.md](./architecture/maritime-watch.md) |
| `AIS_MOCK_ENABLED`             | No       | `true` or `false` (default `false`). When true at boot, starts the Galaxy Leader mock feeder. Operators can also toggle it at runtime from the watch toolbar (`mockAisSetEnabled`)                                                                                                 |
